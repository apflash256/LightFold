...
namespace lightfold {
    ...
    struct BVHPrimitiveInfo {
        ...
        BVHPrimitiveInfo(size_t primitiveNumber, const Bounds3f& bounds)
            ...
            centroid(Lerp(0.5f, bounds.pMin, bounds.pMax)) { }
    ...
    BVHAccel::BVHAccel(std::vector<std::shared_ptr<Primitive>> p,
        ...
        nodes = (LinearBVHNode*)_aligned_malloc(sizeof(LinearBVHNode) * totalNodes, 64);
        int offset = 0;
    ...
    BVHBuildNode* BVHAccel::HLBVHBuild(const std::vector<BVHPrimitiveInfo>& primitiveInfo,
        ...
        for (int start = 0, end = 1; end <= (int)mortonPrims.size(); ++end) {
            ...
            if (end == (int)mortonPrims.size() ||
                ...
                BVHBuildNode* nodes = (BVHBuildNode*)malloc(sizeof(BVHBuildNode) * maxBVHNodes);
    ...
    BVHBuildNode* BVHAccel::buildUpperSAH( std::vector<BVHBuildNode*>& treeletRoots,
        ...
        Bounds3f centroidBounds;
        for (int i = start; i < end; ++i) {
            Point3f centroid = Lerp(0.5f, treeletRoots[i]->bounds.pMin,
                treeletRoots[i]->bounds.pMax);
    ...
    BVHAccel::~BVHAccel() { _aligned_free(nodes); }

    bool BVHAccel::Intersect(const Ray& ray, SurfaceInteraction* isect) const {
        if (!nodes) return false;
        ProfilePhase p(Prof::AccelIntersect);
        bool hit = false;
        Tangent3f invDir(1 / ray.d.x, 1 / ray.d.y, 1 / ray.d.z);
        ...
        while (true) {
            ...
            if (node->bounds.IntersectP(ray.o, ray.d, ray.tMax, invDir, dirIsNeg)) {
                if (node->nPrimitives > 0) {
    ...
    bool BVHAccel::IntersectP(const Ray& ray) const {
        ...
        Tangent3f invDir(1.f / ray.d.x, 1.f / ray.d.y, 1.f / ray.d.z);
        int dirIsNeg[3] = { invDir.x < 0, invDir.y < 0, invDir.z < 0 };
        int nodesToVisit[64];
        int toVisitOffset = 0, currentNodeIndex = 0;
        while (true) {
